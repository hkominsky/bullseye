...
import { ReactComponent as EyeOffIcon } from '../../../assets/svg/eye-off-icon.svg';
import AuthLayout from '../widgets/auth-layout.tsx';
import userAuthService from '../../../services/auth.ts';
import { LoginFormData } from '../utils/types.ts';
...
function Login() {
  ...
  const handleGoogleLogin = (): void => {
    try {
      setError('');
      userAuthService.initiateGoogleAuth();
    } catch (error) {
  ...
  const handleGitHubLogin = (): void => {
    try {
      setError('');
      userAuthService.initiateGitHubAuth();
    } catch (error) {
   ...
   */
  const configureSessionTimeout = (): void => {
    if (!formData.rememberMe) {
      userAuthService.setSessionTimeout(sessionLimit);
    }
  };
